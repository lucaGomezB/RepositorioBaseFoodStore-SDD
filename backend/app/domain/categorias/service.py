...
from app.domain.categorias.schemas import CategoriaCreate, CategoriaUpdate, CategoriaTree


class CategoriaService:
    """Service for Categoria operations following Router -> Service -> Repository pattern."""

    def __init__(self, uow: "UnitOfWork", repo: Optional[CategoriaRepository] = None):
        self.uow = uow
        self.repo = repo or CategoriaRepository(uow.session)
    ...
    def get_tree(self, include_deleted: bool = False) -> list[CategoriaTree]:
        ...
        categories = list(self.uow.session.exec(statement))

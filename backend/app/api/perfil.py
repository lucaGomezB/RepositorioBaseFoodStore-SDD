...
from app.domain.perfil.schemas import PerfilResponse, PerfilUpdate, PasswordChange
...
def get_perfil(
    ...
    with UnitOfWork(session) as uow:
        return PerfilService.get_profile(uow, current_user.user_id)
...
def update_perfil(
    ...
    with UnitOfWork(session) as uow:
        return PerfilService.update_profile(
            uow, current_user.user_id, data.model_dump(exclude_unset=True)
        )
